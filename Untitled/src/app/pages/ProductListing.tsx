...
export function ProductListing() {
    ...
    () =>
      products.map((product) => {
        ...
        return {
          ...product,
          discount,
          isDeal: product.isDeal ?? discount > 0,
          badge: product.badge ?? (discount >= 30 ? "Hot Deal" : discount > 0 ? "Sale" : undefined),
        };
      }),
    [products]
  );
  ...
  const filteredProducts = useMemo(
    () =>
      productsWithMetadata.filter((product) => {
        ...
        if (selectedPriceRange !== "all") {
          ...
          if (selectedPriceRange === "0-10000" && price >= 10000) return false;
          if (selectedPriceRange === "10000-25000" && (price < 10000 || price >= 25000)) return false;
          if (selectedPriceRange === "25000-50000" && (price < 25000 || price >= 50000)) return false;
        ...
        if (searchTerm) {
          ...
          const inBadge = (product.badge || "").toLowerCase().includes(q);
          if (!(inName || inCategory || inBadge)) return false;
        }

        return true;
      }),
    [productsWithMetadata, selectedCategory, selectedPriceRange, searchTerm]
  );

  const dealSorter = (a: typeof productsWithMetadata[number], b: typeof productsWithMetadata[number]) => {
  ...
  const getVisibleProducts = (items: typeof productsWithMetadata) => {
    if (sortBy === "price-low") {
  ...
  const visibleProducts = useMemo(
    () => getVisibleProducts(filteredProducts),
    [filteredProducts, sortBy, showDeals]
  );
                  ...
                  >
                    ...
                    >
                      <div className="relative aspect-square overflow-hidden bg-gradient-to-br from-slate-800 to-slate-900">
                        ...
                        {product.isDeal && (
                          <motion.span
                            ...
                            className={`absolute top-3 left-3 px-3 py-1 backdrop-blur-sm text-white text-xs rounded-full font-bold ${
                              product.badge === "Hot Deal"
                                ? "bg-red-500/80"
                                : "bg-orange-500/80"
                            }`}
                          >
                            {product.badge ?? "HOT DEAL"} {product.discount ? `${product.discount}% OFF` : ""}
                          </motion.span>
                        )}
                      </div>
                      ...
                      <div className="p-4">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-xl text-white font-bold">{formatINR(product.price)}</span>
                          {product.originalPrice && (
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="p-6 flex-1 flex flex-col justify-between">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <span className="text-2xl text-white font-bold">{formatINR(product.price)}</span>
                            {product.originalPrice && (
                              <span className="text-sm text-white/50 line-through">
                                {formatINR(product.originalPrice)}
                              </span>
